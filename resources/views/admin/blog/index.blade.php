...
@section('page-banner')
    <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Blog Yönetimi</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Blog yazılarını yönetin</p>
    </div>
                ...
                <tbody id="sortable-body" class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($blogs as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" data-id="{{ $item->id }}" id="row-{{ $item->id }}">
                            ...
                            <td class="px-6 py-4">
                                <x-action-menu>
                                    <a href="{{ route('blogs.edit', $item->id) }}" class="am-item">
                                    ...
                                    @foreach ($activeLanguages as $activeLang)
                                        <a href="{{ route('blogs.editTranslate', ['id' => $item->id, 'lang' => $activeLang->locale]) }}" class="am-item">
                                    ...
                                    <form action="{{ route('blogs.delete', $item->id) }}" method="POST"
                                          x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Blog Yazısını Sil', message: 'Bu blog yazısını silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-slate-500 dark:text-slate-400">Henüz blog yazısı eklenmemiş.</p>
                                    <a href="{{ route('blogs.create') }}" class="text-fuchsia-600 dark:text-fuchsia-400 hover:underline text-sm font-medium">
                                        İlk blog yazısını ekleyin
                                    </a>
                                </div>
                            </td>
                        </tr>
...
<script>
    ...
    if (sortableBody) {
        Sortable.create(sortableBody, {
            ...
            onEnd: function () {
                ...
                axios.post('{{ route("blogs.updateOrder") }}', {
                    order: order,
                    _token: csrfToken
                })
    ...
    document.querySelectorAll('.status-toggle').forEach(function (checkbox) {
        checkbox.addEventListener('change', function () {
            ...
            axios.post('/panel/blogs/' + itemId + '/toggle', { _token: csrfToken })
            .then(function (response) {
                if (response.data.status === 1) {
                    showToast('success', response.data.message);
