...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('blogTags.edit', $tag->id)" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Etiket Çeviri</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $selectedLanguage->name ?? $selectedLang }} dilinde çeviri yapın</p>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
<form action="{{ route('blogTags.updateTranslate', $tag->id) }}" method="POST">
    @csrf
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Ad</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $tag->getTranslation('name', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="relative flex items-center gap-3 px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Etiket adının çevirisini yazın</p>
                    </div>
                </div>

                <div class="relative p-6 space-y-5">
                    <x-text-input name="name" label="Ad" placeholder="Etiket adının çevirisini yazın..." :value="$tag->getTranslation('name', $selectedLang, false)" ringColor="blue" required />
                </div>
            </div>
        </div>
        ...
        <div class="space-y-5 sticky top-24 self-start">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-5 space-y-3">
                ...
                <a href="{{ route('blogTags.edit', $tag->id) }}"
                   class="w-full inline-flex items-center justify-center gap-2 px-5 py-3
