...
@section('page-banner')
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">
        @yield('page-title', 'Blog Kategori' . (isset($selectedLanguage) && $selectedLanguage ? ' - ' . $selectedLanguage : ''))
    </h1>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div class="rounded-lg mb-4">
            ...
            <div class="w-full bg-white py-10 px-8 rounded-lg h-full">
                <form class="w-1/2 lg:w-full" action="{{ route('blogs.categories.store') }}" method="POST"
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 h-fit">
                        ...
                        <tbody>
                            @foreach ($categories as $item)
                                <tr
                                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 h-fit">
                                    <th class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap h-fit">
                                        <a href="{{ route('blogs.categories.edit', $item->id) }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                            {{ $item->name }}
                                        </a>
                                    </th>
                                    <td class="px-6 py-4 h-fit">
                                        {{ $item->description }}
                                    </td>
                                    <td class="px-6 py-4 h-fit">
                                        {{ $item->blogs_count }}
