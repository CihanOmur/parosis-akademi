...
@section('page-banner')
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">
        @yield('page-title', 'Blog Güncelle' . (isset($selectedLanguage) && $selectedLanguage ? ' - ' . $selectedLanguage : ''))
    </h1>
    <div class="flex items-center gap-2">
        <a href="{{ route('blogs.create') }}" class="bg-blue-500 text-white font-bold py-2 px-4 rounded cursor-pointer">Yeni
            Blog Ekle</a>
    </div>
@endsection
@section('styles')
...
@section('content')
    <div class="rounded-lg mb-4">
        ...
        <div class="w-full bg-white py-10 px-8 rounded-lg">
            <form class="lg:w-1/2 w-full" action="{{ route('blogs.update', $blog->id) }}" method="POST"
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="title" id="title" aria-describedby="helper-text-explanation"
                        ...
                        placeholder="Başlık girin" value="{{ translateAttribute($blog, 'title', request()->lang) }}">
                </div>
                <div class="mb-6">
                    ...
                    <textarea id="editor" name="description"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Açıklama girin">{{ translateAttribute($blog, 'description', request()->lang) }}</textarea>
                </div>
                ...
                <div id="preview_file" class="mb-6 ">
                    <img id="preview_image" src="{{ $blog->image_url ? asset('storage/blogs/' . $blog->image_url) : '' }}"
