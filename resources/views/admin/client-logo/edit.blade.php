...
@section('page-banner')
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Logo Düzenle</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Logo bilgilerini düzenleyin</p>
        </div>
    </div>
@endsection

@section('content')
<form action="{{ route('client-logos.update', $logo->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="relative flex items-center gap-3 px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Logo görselini ve bilgilerini düzenleyin</p>
                    </div>
                </div>
                ...
                <div class="relative p-6 space-y-5">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name="name" id="name"
                               ...
                               placeholder="Firma adını yazın (opsiyonel)..."
                    ...
                    <div class="space-y-1" x-data="{ preview: null }">
                        ...
                        @if($logo->image)
                            <div class="mb-3 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl flex items-center justify-center" x-show="!preview">
                                <img src="{{ asset($logo->image) }}" class="h-16 max-w-[200px] object-contain">
                            </div>
                        @endif
                        ...
                        <input type="file" name="image" id="image" accept="image/*"
                               ...
                               class="w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4
                                      ...
                                      cursor-pointer">
                        <p class="text-xs text-slate-400 mt-1">Yeni görsel yüklerseniz mevcut değiştirilir. PNG, JPG, SVG veya WEBP. Maks 2MB.</p>
                        @error('image')
                            <p class="text-sm text-red-500 flex items-center gap-1.5">
                                <svg class="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M18 10a8 8 0 1 1-16 0 8 8 0 0 1 16 0Zm-8-5a.75.75 0 0 1 .75.75v4.5a.75.75 0 0 1-1.5 0v-4.5A.75.75 0 0 1 10 5Zm0 10a1 1 0 1 0 0-2 1 1 0 0 0 0 2Z" clip-rule="evenodd"/>
