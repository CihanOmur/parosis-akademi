...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('competitions.index')" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Yeni Yarışma</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Öğrencilere atanabilecek bir yarışma ekleyin</p>
            </div>
        </div>
    </div>
@endsection
...
<form action="{{ route('competitions.store') }}" method="POST">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-5">
            @include('admin.competitions.partials.form-fields')
        </div>
