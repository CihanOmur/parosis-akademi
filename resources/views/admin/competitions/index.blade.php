                ...
                <div data-id="{{ $item->id }}"
                    ...
                    <div class="px-5 py-3 bg-slate-50 dark:bg-slate-900/40 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <x-action-menu>
                                <a href="{{ route('competitions.edit', $item->id) }}" class="am-item">
                                ...
                                <form action="{{ route('competitions.delete', $item->id) }}" method="POST"
                                      x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Yarışmayı Sil', message: '{{ addslashes($item->name) }} yarışmasını silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="am-item am-danger">
                                        <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79"/></svg>
                                        Sil
                                    </button>
                                </form>
                            </x-action-menu>
