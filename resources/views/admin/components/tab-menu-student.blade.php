<div class="flex items-center gap-1 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 p-1.5">
    ...
    <a href="{{ route('students.allPayments', $student->id) }}"
       class="inline-flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-xl transition-all
              {{ Route::is('students.allPayments') ? 'bg-gradient-to-r from-fuchsia-500 to-purple-500 text-white shadow-sm shadow-fuchsia-500/20' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50' }}">
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
