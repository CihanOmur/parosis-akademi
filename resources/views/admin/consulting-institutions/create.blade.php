...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('consultingInstitutions.index')" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Yeni Danışmanlık Kurumu</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Sertifikalarda seçilebilecek bir kurum ekleyin</p>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
<form action="{{ route('consultingInstitutions.store') }}" method="POST">
    @csrf
        ...
        <div class="lg:col-span-2 space-y-5">
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="relative p-6 space-y-5">
                    <x-text-input name="name" label="Kurum Adı" placeholder="Örn: ABC Eğitim Kurumu" required />

                    <div class="border-t border-dashed border-slate-200 dark:border-slate-700/60"></div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <x-text-input type="email" name="contact_email" label="İletişim E-posta (opsiyonel)" placeholder="info@kurum.com" />
                        <x-text-input type="tel" name="contact_phone" label="İletişim Telefon (opsiyonel)" placeholder="+90 ..." />
                    </div>

                    <div class="border-t border-dashed border-slate-200 dark:border-slate-700/60"></div>

                    <x-textarea name="notes" label="Notlar (opsiyonel)" placeholder="Kurum hakkında dahili notlar..." rows="4" />
                </div>
            </div>
        </div>
