...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('coupons.index')" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Kuponu Düzenle</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $coupon->code }}</p>
            </div>
        </div>
    </div>
@endsection
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="relative flex items-center gap-3 px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Kupon kodu ve indirim detaylarını düzenleyin</p>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <select name="type" id="type"
                                ...
                                <option value="fixed" {{ old('type', $coupon->type) === 'fixed' ? 'selected' : '' }}>Sabit TL (₺)</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
        ...
        <div class="space-y-5 sticky top-24 self-start">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-5">
                <h4 class="text-sm font-semibold text-slate-900 dark:text-white mb-4">Kullanım İstatistikleri</h4>
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-500 dark:text-slate-400">Kullanım Sayısı</span>
                        <span class="text-sm font-bold text-slate-900 dark:text-white">{{ $coupon->used_count }}</span>
                    </div>
                    @if($coupon->usage_limit)
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="text-sm text-slate-500 dark:text-slate-400">Limit</span>
                            <span class="text-sm font-bold text-slate-900 dark:text-white">{{ $coupon->usage_limit }}</span>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-500 dark:text-slate-400">Limit</span>
                        <span class="text-sm text-slate-400">Sınırsız</span>
                    </div>
                    @endif
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-500 dark:text-slate-400">Durum</span>
                        @if($coupon->is_active)
                            <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400">Aktif</span>
                        @else
                            <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400">Pasif</span>
                        @endif
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-500 dark:text-slate-400">Oluşturulma</span>
                        <span class="text-xs text-slate-400">{{ $coupon->created_at->format('d.m.Y H:i') }}</span>
                    </div>
                </div>
            </div>
        </div>
