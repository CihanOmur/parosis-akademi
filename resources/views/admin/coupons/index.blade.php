                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($coupons as $coupon)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <x-action-menu>
                                    <a href="{{ route('coupons.edit', $coupon->id) }}" class="am-item">
                                    ...
                                    <form action="{{ route('coupons.delete', $coupon->id) }}" method="POST"
                                          x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Kuponu Sil', message: '{{ $coupon->code }} kuponunu silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                        @csrf
                                        @method('DELETE')
