                    ...
                    @forelse($applications as $a)
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                            ...
                            <td class="px-4 py-3 text-right whitespace-nowrap">
                                @if($a->status !== 'contacted')
                                    <form action="{{ route('course-applications.contacted', $a->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="rounded-lg bg-emerald-50 dark:bg-emerald-900/40 px-3 py-1.5 text-xs font-medium text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100" title="İletişime geçildi olarak işaretle">
                                            ✓ İşaretle
                                        </button>
                                    </form>
                                @endif
                                <form action="{{ route('course-applications.destroy', $a->id) }}" method="POST" class="inline" onsubmit="return confirm('Bu başvuruyu silmek istediğinize emin misiniz?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="rounded-lg bg-red-50 dark:bg-red-900/40 px-3 py-1.5 text-xs font-medium text-red-700 dark:text-red-300 hover:bg-red-100" title="Sil">
                                        🗑
                                    </button>
                                </form>
                            </td>
