...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('courseCategories.index')" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Kategori Düzenle</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Kurs kategorisini düzenleyin</p>
            </div>
        </div>
    </div>
@endsection
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-5">
                ...
                <div class="space-y-2">
                    @foreach ($activeLanguages as $activeLang)
                        <a href="{{ route('courseCategories.editTranslate', ['id' => $category->id, 'lang' => $activeLang->locale]) }}"
