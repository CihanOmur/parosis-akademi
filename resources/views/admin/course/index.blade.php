...
@section('page-banner')
    <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Kurs Yönetimi</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Kursları yönetin</p>
    </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-slate-50 dark:bg-slate-700/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-32">Fiyat</th>
                ...
                <tbody id="sortable-body" class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($courses as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" data-id="{{ $item->id }}" id="row-{{ $item->id }}">
                            ...
                            <td class="px-6 py-4">
                                @if($item->image)
                                    <img src="{{ asset($item->image) }}" alt="{{ $item->title }}" class="w-10 h-10 rounded-lg object-cover">
                                @else
                            ...
                            <td class="px-6 py-4">
                                <x-action-menu>
                                    <a href="{{ route('courses.edit', $item->id) }}" class="am-item">
                                    ...
                                    @foreach ($activeLanguages as $activeLang)
                                        <a href="{{ route('courses.editTranslate', ['id' => $item->id, 'lang' => $activeLang->locale]) }}" class="am-item">
                                    ...
                                    <form action="{{ route('courses.delete', $item->id) }}" method="POST"
                                          x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Kursu Sil', message: 'Bu kursu silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-slate-500 dark:text-slate-400">Henüz kurs eklenmemiş.</p>
                                    <a href="{{ route('courses.create') }}" class="text-fuchsia-600 dark:text-fuchsia-400 hover:underline text-sm font-medium">
                                        İlk kursu ekleyin
                                    </a>
                                </div>
                            </td>
...
<script>
    ...
    if (sortableBody) {
        Sortable.create(sortableBody, {
            ...
            onEnd: function () {
                ...
                axios.post('{{ route("courses.updateOrder") }}', {
                    order: order,
                    _token: csrfToken
                })
    ...
    document.querySelectorAll('.status-toggle').forEach(function (checkbox) {
        checkbox.addEventListener('change', function () {
            ...
            axios.post('/panel/courses/' + itemId + '/toggle', { _token: csrfToken })
            .then(function (response) {
                if (response.data.status === 1) {
                    showToast('success', response.data.message);
