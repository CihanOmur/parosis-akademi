...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('languages.index')" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Dil Düzenle</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400"><span class="font-mono font-semibold text-fuchsia-600 dark:text-fuchsia-400">{{ $language->locale }}</span> dilini düzenleyin</p>
            </div>
        </div>
    </div>
@endsection
        ...
        <div class="space-y-5 sticky top-24 self-start">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-5 space-y-3">
                ...
                @role('SuperAdmin')
                    ...
                    <button type="submit" form="language-delete-form"
                            class="w-full inline-flex items-center justify-center gap-2 px-5 py-3
...
@role('SuperAdmin')
@if(!$language->is_default)
<form id="language-delete-form" action="{{ route('languages.delete', $language->id) }}" method="POST"
