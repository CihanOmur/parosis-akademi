                ...
                <tbody id="sortable-body" class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($languages as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" data-id="{{ $item->id }}" id="row-{{ $item->id }}">
                            ...
                            <td class="px-6 py-4">
                                <x-action-menu>
                                    <a href="{{ route('languages.edit', $item->id) }}" class="am-item">
                                    ...
                                    @role('SuperAdmin')
                                        @if(!$item->is_default)
                                            ...
                                            <form action="{{ route('languages.delete', $item->id) }}" method="POST"
                                                  x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Dili Sil', message: 'Bu dili silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                                @csrf
                                                @method('DELETE')
