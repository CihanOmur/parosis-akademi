    ...
    <nav class="sidebar-nav flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        @canany(['content', 'content_delete', 'menu', 'language'])
            <div class="pt-4">
                ...
                <div x-show="sidebarCollapsed"
                     class="hidden lg:block mx-auto w-8 border-t border-slate-200 dark:border-slate-700 mb-2"></div>
                <p class="px-3 text-xs font-medium uppercase text-slate-400 dark:text-slate-500 mb-2 lg:hidden">Görünüm</p>
            </div>
        @endcanany
        ...
        <a href="{{ route('sliders.index') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200
            ...
            <span x-show="!sidebarCollapsed" x-transition class="hidden lg:block">Slider</span>
            <span class="lg:hidden">Slider</span>
        </a>
        @endcanany
        ...
        <div class="group/users relative">
            <button @click="openMenu = openMenu === 'users' ? '' : 'users'"
                ...
                <span x-show="!sidebarCollapsed" x-transition class="flex-1 text-left hidden lg:block">Kullanıcılar</span>
                <span class="flex-1 text-left lg:hidden">Kullanıcılar</span>
                <svg x-show="!sidebarCollapsed"
                     class="w-4 h-4 transition-transform duration-200 hidden lg:block"
            ...
            <div x-show="openMenu === 'users' && (!sidebarCollapsed || window.innerWidth < 1024)"
                 x-collapse
                 class="ml-12 mt-1 space-y-1">
            ...
            <template x-if="sidebarCollapsed">
                <div class="hidden lg:group-hover/users:block absolute left-full top-0 pl-2 z-[60]">
                    <div class="w-52 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 py-2">
                        <p class="px-3 py-1.5 text-xs font-semibold text-slate-400 uppercase tracking-wider">Kullanıcılar</p>
                        <a href="{{ route('users.index') }}"
                           class="block px-3 py-2 text-sm rounded-lg mx-1 transition-colors
                                  {{ Route::is('users.*')
                                      ? 'text-fuchsia-600 dark:text-fuchsia-400 font-medium bg-fuchsia-50 dark:bg-fuchsia-500/10'
