...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('menu-items.edit', $menuItem->id)" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Menu Ogesi Ceviri</h1>
...
@section('content')
<form action="{{ route('menu-items.updateTranslate', $menuItem->id) }}" method="POST">
    @csrf
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Etiket</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $menuItem->getTranslation('label', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-400 uppercase">URL</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1 font-mono">{{ $menuItem->url }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="relative flex items-center gap-3 px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Etiketin cevirisini yazin</p>
                    </div>
                </div>

                <div class="relative p-6 space-y-5">
                    <x-text-input name="label" label="Etiket" placeholder="Etiketin cevirisini yazin..." :value="$menuItem->getTranslation('label', $selectedLang, false)" ringColor="blue" required />
                </div>
            </div>
        </div>
