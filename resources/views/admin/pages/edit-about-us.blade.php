...
@section('content')
    <div class="rounded-lg mb-4">
        ...
        <div class="w-full bg-white py-10 px-8 rounded-lg">
            <form class="lg:w-1/2 w-full" action="{{ route('pages.update', ['id' => 'about-us']) }}" method="POST"
                    ...
                    <select id="references" name="references[]" multiple
                        ...
                        @foreach ($references as $reference)
                            <option value="{{ $reference->id }}"
                                ...
                                {{ strip_tags($reference->name) }}
                                {{ $reference->reference_type == 'customer' ? ' - Müşteri' : ' - Partner' }}</option>
                        @endforeach
                    </select>
                </div>
                <script>
                    new TomSelect('#references', {
                        create: false,
                        highlight: true,
                        persist: false,
                        openOnFocus: true,
                ...
                <div>
                    ...
                    <div id="project-gallery-wrapper" class="space-y-4">
                        ...
                        @if ($aboutUsPageGallery->isEmpty())
                            <div class="project-gallery-item flex flex-col gap-4" data-gallery-id="0">
                                <div class="flex items-center gap-4">
                                    <div class="w-1/3">
                                        <x-text-input name="gallery_items[0][title]" placeholder="Başlık" ringColor="blue" />
                                    </div>
                        ...
                        @else
                            @foreach ($aboutUsPageGallery as $gallery)
                                <div class="project-gallery-item flex flex-col gap-4"
                                    data-gallery-id="{{ $loop->index }}">
                                    <div class="flex items-center gap-4">
                                        <div class="w-1/3">
                                            <x-text-input name="gallery_items[{{ $loop->index }}][title]" placeholder="Başlık" ringColor="blue"
    ...
    <script>
        let galleryIndex = {{ $aboutUsPageGallery->count() + 1 }};
