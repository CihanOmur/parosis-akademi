...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('productCategories.index')" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Kategori Düzenle</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Ürün kategorisini düzenleyin</p>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
<form action="{{ route('productCategories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="relative flex items-center gap-3 px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Varsayılan dilde kategori adını düzenleyin</p>
                    </div>
                </div>
                ...
                <div class="relative p-6 space-y-5">
                    ...
                    <div class="space-y-1">
                        ...
                        <x-image-upload name="image" uploadPath="product-categories" :existing="$category->image ? asset($category->image) : null" />
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-5">
                ...
                <div class="space-y-2">
                    @foreach ($activeLanguages as $activeLang)
                        <a href="{{ route('productCategories.editTranslate', ['id' => $category->id, 'lang' => $activeLang->locale]) }}"
