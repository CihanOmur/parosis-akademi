...
@section('page-banner')
    <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Ürün Kategorileri</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Ürün kategorilerini yönetin</p>
    </div>
                ...
                <tbody id="sortable-body" class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($categories as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" data-id="{{ $item->id }}" id="row-{{ $item->id }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="w-10 h-10 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-700 flex items-center justify-center">
                                    @if($item->image)
                                        <img src="{{ asset($item->image) }}" alt="" class="w-10 h-10 object-cover">
                                    @else
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('productCategories.edit', $item->id) }}"
                            ...
                            <td class="px-6 py-4">
                                <x-action-menu>
                                    <a href="{{ route('productCategories.edit', $item->id) }}" class="am-item">
                                    ...
                                    @foreach ($activeLanguages as $activeLang)
                                        <a href="{{ route('productCategories.editTranslate', ['id' => $item->id, 'lang' => $activeLang->locale]) }}" class="am-item">
                                    ...
                                    <form action="{{ route('productCategories.delete', $item->id) }}" method="POST"
                                          x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Kategoriyi Sil', message: 'Bu ürün kategorisini silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-slate-500 dark:text-slate-400">Henüz ürün kategorisi eklenmemiş.</p>
                                    <a href="{{ route('productCategories.create') }}" class="text-fuchsia-600 dark:text-fuchsia-400 hover:underline text-sm font-medium">
...
<script>
    ...
    if (sortableBody) {
        Sortable.create(sortableBody, {
            ...
            onEnd: function () {
                ...
                axios.post('{{ route("productCategories.updateOrder") }}', {
                    order: order,
                    _token: csrfToken
                })
    ...
    document.querySelectorAll('.status-toggle').forEach(function (checkbox) {
        checkbox.addEventListener('change', function () {
            ...
            axios.post('/panel/product-categories/' + itemId + '/toggle', { _token: csrfToken })
            .then(function (response) {
                if (response.data.status === 1) {
                    showToast('success', response.data.message);
