...
@section('page-banner')
    <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Ürünler</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Ürünleri yönetin</p>
    </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-slate-50 dark:bg-slate-700/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-32">SKU</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-40">Fiyat</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-24">Stok</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Kategoriler</th>
                ...
                <tbody id="sortable-body" class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($products as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" data-id="{{ $item->id }}" id="row-{{ $item->id }}">
                            ...
                            <td class="px-6 py-4">
                                @if($item->image)
                                    <img src="{{ asset($item->image) }}" alt="{{ $item->name }}" class="w-12 h-12 rounded-lg object-cover">
                                @else
                            ...
                            <td class="px-6 py-4">
                                @if($item->sale_price)
                                    <div class="flex flex-col">
                                        <span class="font-semibold text-slate-900 dark:text-white">₺{{ number_format($item->sale_price, 2) }}</span>
                            ...
                            <td class="px-6 py-4 text-slate-600 dark:text-slate-300">
                                @if($item->manage_stock)
                                    <span class="{{ $item->stock > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400' }} font-medium">
                                        {{ $item->stock ?? 0 }}
                                    </span>
                                @else
                                    <span class="text-slate-400">—</span>
                                ...
                                @if($item->categories->count())
                                    <div class="flex flex-wrap gap-1">
                                        @foreach($item->categories as $cat)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-fuchsia-50 text-fuchsia-700 dark:bg-fuchsia-900/20 dark:text-fuchsia-400">{{ $cat->name }}</span>
                                        @endforeach
                                    </div>
                                @else
                                    —
                            ...
                            <td class="px-6 py-4">
                                <x-action-menu>
                                    <a href="{{ route('products.edit', $item->id) }}" class="am-item">
                                    ...
                                    @foreach ($activeLanguages as $activeLang)
                                        <a href="{{ route('products.editTranslate', ['id' => $item->id, 'lang' => $activeLang->locale]) }}" class="am-item">
                                    ...
                                    <form action="{{ route('products.delete', $item->id) }}" method="POST"
                                          x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Ürünü Sil', message: 'Bu ürünü silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="9" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <a href="{{ route('products.create') }}" class="text-fuchsia-600 dark:text-fuchsia-400 hover:underline text-sm font-medium">
                                        İlk ürünü ekleyin
                                    </a>
                                </div>
                            </td>
...
<script>
    ...
    if (sortableBody) {
        Sortable.create(sortableBody, {
            ...
            onEnd: function () {
                ...
                axios.post('{{ route("products.updateOrder") }}', {
                    order: order,
                    _token: csrfToken
                })
    ...
    document.querySelectorAll('.status-toggle').forEach(function (checkbox) {
        checkbox.addEventListener('change', function () {
            ...
            axios.post('/panel/products/' + itemId + '/toggle', { _token: csrfToken })
            .then(function (response) {
                if (response.data.status === 1) {
                    showToast('success', response.data.message);
