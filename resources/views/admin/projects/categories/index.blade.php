...
@section('page-banner')
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">
        @yield('page-title', 'Proje Kategorileri' . (isset($selectedLanguage) && $selectedLanguage ? ' - ' . $selectedLanguage : ''))
    </h1>
    <div class="flex items-center gap-2">
        <a href="{{ route('projects.create') }}"
            class="bg-blue-500 text-white font-bold py-2 px-4 rounded cursor-pointer">Yeni Proje</a>
    </div>
@endsection
@section('styles')
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div class="rounded-lg mb-4">
            ...
            <div class="w-full bg-white py-10 px-8 rounded-lg">
                <form class="w-1/2 lg:w-full" action="{{ route('projects.categories.store') }}" method="POST"
                            ...
                            @foreach ($categories as $item)
                                <tr
                                    ...
                                    <th scope="row"
                                        ...
                                        <a href="{{ route('projects.categories.edit', $item->id) }}"
                                    ...
                                    <td class="px-6 py-4 text-right flex gap-2 items-center">
                                        ...
                                        <form action="{{ route('projects.categories.delete', $item->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                            ...
                                            <div id="languageDropdown{{ $item->id }}"
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 text-start"
                                                    ...
                                                    @foreach ($activeLanguages as $activeLanguageItem)
                                                        <li>
                                                            <a href="{{ route('projects.categories.categoryEditTranslate', ['lang' => $activeLanguageItem->locale, 'id' => $item->id]) }}"
