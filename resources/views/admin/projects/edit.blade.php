...
@section('content')
    <div class="rounded-lg mb-4">
        ...
        <div class="w-full bg-white py-10 px-8 rounded-lg">
            <form id="myForm" class="lg:w-1/2 w-full" action="{{ route('projects.update', $project->id) }}" method="POST"
                ...
                <div id="preview_file" class="mb-6 ">
                    <img id="preview_image"
                        src="{{ $project->image_url ? asset('storage/projects/' . $project->image_url) : '' }}"
                ...
                <div id="personal-info-wrapper" class="space-y-4 ">
                    @foreach ($project->infoItems as $info)
                        <div class="personal-info-item flex items-center gap-4">
                            <div class="w-1/3">
                                <input type="text" name="info_items[{{ $loop->index }}][title]"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    placeholder="Başlık" value="{{ translateAttribute($info, 'title', request()->lang) }}">
                            </div>

                            <div class="w-2/3">
                                <input type="text" name="info_items[{{ $loop->index }}][description]"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    placeholder="Açıklama"
                                    value="{{ translateAttribute($info, 'description', request()->lang) }}">
                            </div>
                ...
                <div id="project-gallery-wrapper" class="space-y-4">
                    ...
                    @foreach ($project->gallery as $gallery)
                        <div class="project-gallery-item flex flex-col gap-4" data-gallery-id="{{ $loop->index }}">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="w-2/3 relative">
                                    ...
                                    <input type="hidden" name="gallery_items[{{ $loop->index }}][uploaded_file]"
                ...
                <div id="gallery-preview-section" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
                    @foreach ($project->gallery as $gallery)
                        <div class="gallery-preview-item" data-gallery-id="{{ $loop->index }}">
                            <div class="relative">
                                @if ($gallery->is_image)
                                    <img src="{{ asset('storage/projects/' . $gallery->file_url) }}" alt="Gallery Image"
                                        class="w-full max-w-xs max-h-[200px] object-contain rounded shadow">
                                @else
                                    <video controls class="w-full max-w-xs max-h-[200px] object-contain rounded shadow">
                                        <source src="{{ asset('storage/projects/' . $gallery->file_url) }}"
    ...
    <script>
        let galleryIndex = {{ $project->gallery->count() + 1 }};
    ...
    <script>
        let index = {{ $project->infoItems->count() }};
