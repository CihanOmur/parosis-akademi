...
@section('page-banner')
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">
        @yield('page-title', 'Proje Listesi' . (isset($selectedLanguage) && $selectedLanguage ? ' - ' . $selectedLanguage : ''))
    </h1>
                        ...
                        @foreach ($projects as $item)
                            <tr
                                ...
                                <th scope="row"
                                    ...
                                    <a href="{{ route('projects.edit', $item->id) }}">{{ $item->title }}</a>
                                </th>
                                <td class="px-6 py-4">
                                    {{ $item->category->name }}
                                </td>
                                            ...
                                            <div id="languageDropdown{{ $item->id }}"
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 text-start"
                                                    ...
                                                    @foreach ($activeLanguages as $activeLanguageItem)
                                                        <li>
                                                            <a href="{{ route('projects.editTranslate', ['lang' => $activeLanguageItem->locale, 'id' => $item->id]) }}"
