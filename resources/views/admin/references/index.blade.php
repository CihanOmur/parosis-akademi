...
@section('page-banner')
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">
        @yield('page-title', 'Referans Listesi' . (isset($selectedLanguage) && $selectedLanguage ? ' - ' . $selectedLanguage : ''))
    </h1>
                        ...
                        @foreach ($references as $item)
                            <tr
                                ...
                                <th scope="row" class="px-6 py-4">
                                    <img src="{{ asset('storage/references/' . $item->image_url) }}"
                                        alt="{{ $item->name }}" class="w-16 h-16 object-cover">
                                </th>

                                <th scope="row"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <a href="{{ route('references.edit', $item->id) }}"
                                ...
                                <td class="px-6 py-4">
                                    {{ $item->reference_type == 'customer' ? 'Müşteri' : 'Ortak' }}
                                </td>
                                <td class="px-6 py-4 gap-2 ">
                                    <div class="flex items-center gap-2">


                                        <form action="{{ route('references.delete', $item->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                            ...
                                            <div id="languageDropdown{{ $item->id }}"
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 text-start"
                                                    ...
                                                    @foreach ($activeLanguages as $activeLanguageItem)
                                                        <li>
                                                            <a href="{{ route('references.editTranslate', ['lang' => $activeLanguageItem->locale, 'id' => $item->id]) }}"
