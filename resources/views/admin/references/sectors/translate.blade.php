...
@section('page-banner')
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">
        @yield('page-title', 'Sektör Listesi' . (isset($selectedLanguage) && $selectedLanguage ? ' - ' . $selectedLanguage : ''))
    </h1>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div class="rounded-lg mb-4">
            ...
            <div class="w-full bg-white py-10 px-8 rounded-lg">
                <form class="w-1/2 lg:w-full" action="{{ route('references.sectors.sectorsUpdateTranslate', $sector->id) }}"
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="title" id="title" aria-describedby="helper-text-explanation"
                            ...
                            placeholder="Başlık girin" value="{{ translateAttribute($sector, 'name', request()->lang) }}">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="editor" rows="4"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="Açıklama girin">{{ translateAttribute($sector, 'description', request()->lang) }}</textarea>
                            ...
                            @foreach ($sectors as $item)
                                <tr
                                    ...
                                    <th scope="row"
                                        ...
                                        <a href="{{ route('references.sectors.edit', $item->id) }}"
                                    ...
                                    <td class="px-6 py-4 text-right flex gap-2 items-center">
                                        ...
                                        <form action="{{ route('references.sectors.delete', $item->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                            ...
                                            <div id="languageDropdown{{ $item->id }}"
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 text-start"
                                                    ...
                                                    @foreach ($activeLanguages as $activeLanguageItem)
                                                        <li>
                                                            <a href="{{ route('references.sectors.sectorsEditTranslate', ['lang' => $activeLanguageItem->locale, 'id' => $item->id]) }}"
