                ...
                <tbody id="sortable-body" class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($sliders as $slider)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" data-id="{{ $slider->id }}" id="row-{{ $slider->id }}">
                            ...
                            <td class="px-6 py-4">
                                <x-action-menu>
                                    <a href="{{ route('sliders.edit', $slider->id) }}" class="am-item">
                                    ...
                                    <a href="{{ route('sliders.items.index', $slider->id) }}" class="am-item">
                                    ...
                                    <form action="{{ route('sliders.delete', $slider->id) }}" method="POST"
                                          x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Slider\'i Sil', message: 'Bu slider grubunu silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                        @csrf
                                        @method('DELETE')
