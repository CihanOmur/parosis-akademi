...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('sliders.items.edit', [$slider->id, $item->id])" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Slayt Çeviri</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $selectedLanguage->name ?? $selectedLang }} dilinde çeviri yapın — {{ $slider->name }}</p>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
<form action="{{ route('sliders.items.updateTranslate', [$slider->id, $item->id]) }}" method="POST">
    @csrf
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    <div>
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $item->getTranslation('title', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Vurgulu Metin</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $item->getTranslation('highlight_text', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Açıklama</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1 whitespace-pre-line">{{ $item->getTranslation('description', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Buton Metni</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $item->getTranslation('button_text', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="relative flex items-center gap-3 px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Slayt metin alanlarının çevirisini yazın</p>
                    </div>
                </div>
                ...
                <div class="relative p-6 space-y-5">
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  ...
                                  placeholder="Açıklamanın çevirisini yazın...">{{ old('description', $item->getTranslation('description', $selectedLang, false)) }}</textarea>
                    </div>
