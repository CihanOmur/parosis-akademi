...
<form action="{{ route('students.update', ['id' => $student->id]) }}" method="POST" enctype="multipart/form-data"
      x-data="{
          studentName: '{{ addslashes($student->full_name) }}',
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="guardian1_education_level" @cannot('student') disabled @endcannot
                            ...
                            @foreach ($education_levels as $level)
                                <option {{ ($student->guardians[0]?->education_level ?? '') == $level ? 'selected' : '' }} value="{{ $level }}">{{ $level }}</option>
                            @endforeach
                        </select>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50">
                ...
                <div class="p-6">
                    <div class="flex items-center gap-3">
                        <button type="button" @click="hasAllergy = '1'" @cannot('student') disabled @endcannot
                        ...
                        <button type="button" @click="hasAllergy = '2'" @cannot('student') disabled @endcannot
                                class="flex items-center gap-2.5 px-5 py-3 rounded-xl border-2 text-sm font-medium transition-all duration-200 cursor-pointer"
                                :class="hasAllergy === '2'
                ...
                <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div class="space-y-1">
                        ...
                        <select name="class_id" @cannot('student') disabled @endcannot
                                class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-700/70 border-0 rounded-xl text-sm text-slate-900 dark:text-white ring-1 ring-slate-200 dark:ring-slate-600 focus:ring-2 focus:ring-teal-500/60 transition-all appearance-none @cannot('student') !cursor-not-allowed opacity-60 @endcannot">
                    ...
                    <div class="space-y-1">
                        ...
                        @endphp
                        <select id="registiration_term" name="registiration_term[]" multiple @cannot('student') disabled @endcannot
                                class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-700/70 border-0 rounded-xl text-sm text-slate-900 dark:text-white ring-1 ring-slate-200 dark:ring-slate-600 focus:ring-2 focus:ring-teal-500/60 transition-all @cannot('student') !cursor-not-allowed opacity-60 @endcannot">
        ...
        <div class="space-y-5 sticky top-24 self-start">
                ...
                <div class="relative flex flex-col items-center text-center gap-3">
                    ...
                    <div>
                        ...
                        <span class="inline-flex items-center gap-1 mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium
                            {{ $student->is_active == '1' ? 'bg-white/20 text-white' : 'bg-red-400/30 text-red-100' }}">
                            {{ $student->is_active == '1' ? 'Aktif Öğrenci' : 'Pasif Öğrenci' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-5">
                <h3 class="text-sm font-semibold text-slate-900 dark:text-white mb-3">Kayıt Bilgileri</h3>
                <div class="space-y-2.5">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                            <p class="text-xs text-slate-400">Öğrenci ID</p>
                            <p class="text-sm font-mono font-semibold text-slate-700 dark:text-slate-300">#{{ $student->id }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2.5">
                        <span class="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center flex-shrink-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ \Carbon\Carbon::parse($student->created_at)->format('d.m.Y') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                            <p class="text-xs text-slate-400">Sınıf</p>
                            <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ $student->lessonClass->name }}</p>
                        </div>
                    </div>
                    @endif
                </div>
