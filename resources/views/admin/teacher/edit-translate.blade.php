...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('teachers.edit', $teacher->id)" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Egitmen Ceviri</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $selectedLanguage->name ?? $selectedLang }} dilinde ceviri yapin</p>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
<form action="{{ route('teachers.updateTranslate', $teacher->id) }}" method="POST">
    @csrf
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                <div class="flex items-center gap-3 px-6 pt-5 pb-3 border-b border-slate-200/50 dark:border-slate-700/50">
                    ...
                    <h3 class="text-sm font-semibold text-slate-500 dark:text-slate-400">Orijinal (Varsayilan Dil)</h3>
                </div>
                <div class="p-6 space-y-3">
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Ad Soyad</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $teacher->getTranslation('name', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Unvan</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $teacher->getTranslation('title', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Kisa Aciklama</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1 whitespace-pre-line">{{ $teacher->getTranslation('short_description', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Biyografi</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1 whitespace-pre-line">{{ $teacher->getTranslation('bio', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="relative flex items-center gap-3 px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        <h3 class="text-base font-semibold text-slate-900 dark:text-white">
                            Ceviri: {{ $selectedLanguage->name ?? $selectedLang }}
                        </h3>
                        <p class="text-xs text-slate-400 mt-0.5">Egitmen bilgilerinin cevirisini yazin</p>
                    </div>
                </div>
