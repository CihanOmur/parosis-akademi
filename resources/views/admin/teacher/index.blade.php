                ...
                <tbody id="sortable-body" class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse ($teachers as $item)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" data-id="{{ $item->id }}" id="row-{{ $item->id }}">
                            ...
                            <td class="px-6 py-4">
                                <x-action-menu>
                                    <a href="{{ route('teachers.edit', $item->id) }}" class="am-item">
                                    ...
                                    @foreach ($activeLanguages as $activeLang)
                                        <a href="{{ route('teachers.editTranslate', ['id' => $item->id, 'lang' => $activeLang->locale]) }}" class="am-item">
                                    ...
                                    <form action="{{ route('teachers.delete', $item->id) }}" method="POST"
                                          x-data @submit.prevent="$dispatch('confirm-dialog', { title: 'Eğitmeni Sil', message: 'Bu eğitmeni silmek istediğinize emin misiniz?', form: $el })" class="am-form">
                                        @csrf
                                        @method('DELETE')
