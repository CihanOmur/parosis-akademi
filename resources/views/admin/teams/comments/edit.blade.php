...
@section('page-banner')
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">
        @yield('page-title', 'Yorum Düzenle' . (isset($selectedLanguage) && $selectedLanguage ? ' - ' . $selectedLanguage : ''))
    </h1>
    <div class="flex items-center gap-2">
        <a href="{{ route('teams.comments.create') }}"
            class="bg-blue-500 text-white font-bold py-2 px-4 rounded cursor-pointer">Yeni Yorum
            Ekle</a>
    </div>
@endsection
@section('styles')
...
@section('content')
    <div class="rounded-lg mb-4">
        ...
        <div class="w-full bg-white py-10 px-8 rounded-lg">
            <form class="lg:w-1/2 w-full" action="{{ route('teams.comments.update', $comment->id) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="lang" value="{{ request()->lang ?? app()->getLocale() }}">
                <div class="mb-6">
                    <label for="team_member_id"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kullanıcı
                        Seçiniz</label>
                    <select id="team_member_id" name="team_member_id"
                        ...
                        @foreach ($teamMembers as $teamMember)
                            <option value="{{ $teamMember->id }}"
                                {{ $teamMember->id == $comment->team_user_id ? 'selected' : '' }}>
                                {{ $teamMember->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-6">
                    ...
                    <textarea id="editor" name="comment"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Açıklama girin">{{ $comment->comment }}</textarea>
                </div>
                <div class="">
