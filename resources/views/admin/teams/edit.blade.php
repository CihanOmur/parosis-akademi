...
@section('page-banner')
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">
        @yield('page-title', 'Takım Düzenle' . (isset($selectedLanguage) && $selectedLanguage ? ' - ' . $selectedLanguage : ''))
    </h1>
...
@section('content')
    <div class="rounded-lg mb-4">
        ...
        <div class="w-full bg-white py-10 px-8 rounded-lg">
            <form class="lg:w-1/2 w-full" action="{{ route('teams.update', $team->id) }}" method="POST"
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="title" id="title" aria-describedby="helper-text-explanation"
                        ...
                        placeholder="Başlık girin" value="{{ translateAttribute($team, 'title', request()->lang) }}">
                </div>
                <div class="mb-6">
                    ...
                    <textarea id="editor" name="description"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Açıklama girin">{{ translateAttribute($team, 'description', request()->lang) }}</textarea>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select id="category" name="category"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" {{ $category->id == $team->category_id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="position" id="position" aria-describedby="helper-text-explanation"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Pozisyon girin" value="{{ translateAttribute($team, 'position', request()->lang) }}">
                </div>
                ...
                <div id="preview_file" class="mb-6 ">
                    <img id="preview_image" src="{{ $team->image_url ? asset('storage/teams/' . $team->image_url) : '' }}"
                        alt="Preview" class="rounded-lg w-60 h-auto">
                </div>
                <script>
                    $(document).ready(function() {
                ...
                <div class="mb-6">
                    ...
                    <input type="email" name="email" id="email" aria-describedby="helper-text-explanation"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="E-posta girin" value="{{ $team->email }}">
                </div>
                ...
                <div id="personal-info-wrapper" class="space-y-4 ">
                    @foreach ($team->personalInfos as $info)
                        <div class="personal-info-item flex items-center gap-4">
                            <div class="w-1/3">
                                <input type="text" name="personal_infos[{{ $loop->index }}][title]"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    placeholder="Başlık" value="{{ $info->title }}">
                            </div>

                            <div class="w-2/3">
                                <input type="text" name="personal_infos[{{ $loop->index }}][description]"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    placeholder="Açıklama" value="{{ $info->description }}">
                            </div>
                ...
                <script>
                    let index = {{ $team->personalInfos->count() }};
