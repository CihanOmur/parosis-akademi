...
@section('page-banner')
    <div class="w-full">
        <div class="flex items-center gap-3">
            <x-back-button :href="route('testimonials.edit', $testimonial->id)" />
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Yorum Çeviri</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $selectedLanguage->name ?? $selectedLang }} dilinde çeviri yapın — {{ $testimonial->name }}</p>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
<form action="{{ route('testimonials.updateTranslate', $testimonial->id) }}" method="POST">
    @csrf
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Rol</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">{{ $testimonial->getTranslation('role', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-slate-400 uppercase">Yorum</span>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1 whitespace-pre-line">{{ $testimonial->getTranslation('quote', app()->getLocale(), false) ?: '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="relative flex items-center gap-3 px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Rol ve yorumun çevirisini yazın</p>
                    </div>
                </div>
