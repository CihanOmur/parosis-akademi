...
@php
    ...
    $selectedJson = json_encode(array_map('intval', $selected));
@endphp
...
<div class="relative"
     x-data="{
         ...
         toggle(id) {
             ...
             if (i === -1) {
                 if (this.maxSelect > 0 && this.selected.length >= this.maxSelect) return;
        ...
        <div class="overflow-y-auto" style="max-height: {{ (int) $maxVisible * 44 }}px">
            <template x-for="item in items" :key="item.id">
                <div x-show="item.name.toLowerCase().includes(search.toLowerCase())"
                     ...
                     :class="!selected.includes(item.id) && atMax() ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'"
